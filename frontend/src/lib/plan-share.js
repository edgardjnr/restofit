...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR RestoFit — just the
...
const unitError = () => { throw new Error(t('this isn’t an RestoFit plan file')) }
...
export function parsePlan(raw, destinationUnit = 'kg') {
  ...
  if (!data || typeof data !== 'object' || Array.isArray(data) || !data.opengym_plan || !Array.isArray(data.routines) || !destination) {
    throw new Error(t('this isn’t an RestoFit plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with RestoFit'))} · restofit.restaurantepro.com.br</footer>
</div></body></html>`
}
